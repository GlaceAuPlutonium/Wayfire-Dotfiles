window-open {
        ...
        custom-shader r"
            ...
            float warpedFbm(vec2 p, float t) {
                vec2 q = vec2(fbm(p),
                            fbm(p + vec2(5.2, 1.3)));
                // Précalcul de p + 6.0 * q utilisé 4 fois dans l'original
                vec2 pq = p + 6.0 * q;
                vec2 r = vec2(fbm(pq + vec2(1.7, 9.2) + 0.25 * t),
                            fbm(pq + vec2(8.3, 2.8) + 0.22 * t));
                // Précalcul de p + 5.0 * r utilisé 2 fois
                vec2 pr = p + 5.0 * r;
                vec2 s = vec2(fbm(pr + vec2(3.1, 7.4) + 0.18 * t),
                            fbm(pr + vec2(6.7, 0.9) + 0.2  * t));
                return fbm(p + 6.0 * s);
            }
            vec4 open_color(vec3 coords_geo, vec3 size_geo) {
                ...
                // Précalcul de uv * 2.0 et t * 0.2
                vec2  uv2  = uv * 2.0;
                float t02  = t * 0.2;
                vec2  wq   = vec2(fbm(uv2 + vec2(0.0, t02)),
                        fbm(uv2 + vec2(5.2, t02)));
                vec2  wr   = vec2(fbm(uv2 + 4.0 * wq + vec2(1.7, 9.2)),
                        fbm(uv2 + 4.0 * wq + vec2(8.3, 2.8)));
